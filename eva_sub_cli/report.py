...
def generate_report(validation_results, validation_date, submission_dir, vcf_fasta_analysis_mapping, project_title,
    ...
    from eva_sub_cli.validators.validator import RUN_STATUS_KEY, TRIM_DOWN, PASS

    results_for_report = {k: v for k, v in validation_results.items() if k != 'ready_for_submission_to_eva'}
    non_file_keys = {RUN_STATUS_KEY, PASS, TRIM_DOWN}
    vcf_files = sorted(set([file_name
                            ...
                            if file_name not in non_file_keys
                            ]))
    fasta_files = sorted([file_name for file_name in results_for_report['fasta_check'] if file_name not in non_file_keys])
